...
//         <img class="avatar-tiny" src="${post.user.avatar}"> <strong>${
//                   post.title
//               }</strong>
...
export default class Search {
    constructor() {
        this.resultContainer = document.getElementById("resultContiner");
    ...
    renderResults(searchData) {
        this.resultContainer.innerHTML = `<div class="card-body">
    ...
    >
    <div id="results" class="list-group-item" ></div>
        
    </div>
</div>`;

        const resutsDiv = document.getElementById("results");

        if (searchData.length > 0) {
            searchData.forEach((post) => {
                const postElement = this.createPostElement(post);
                resutsDiv.appendChild(postElement);
            });
        } else {
            resutsDiv.innerHTML = "<p>No results found.</p>";
        }
    }
        ...
        postElement.innerHTML = `
            <div class="row align-items-center">
                ...
                <div class="col text-truncate">
                    <a href="/post/${post.id}" class="text-reset d-block">${
            post.title
        }</a>
